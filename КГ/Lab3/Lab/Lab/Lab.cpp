...
bool createShaderProgram()
{
    ...
    const GLchar fsh[] =
        ...
        "{"
        "   float S = 30;"
        "   vec3 color = vec3(mix(texture(u_map1, v_texCoord), texture(u_map2, v_texCoord), 0.6)) * 1.5;"
...
bool createGrid(size_t N = 100)
{
    ...
    auto attrCount = 4;
    auto vertices = new GLfloat[verticesCount * attrCount];
    auto indices = new GLuint[indicesCount];
    
    for (size_t i = 0; i < N + 1; i++) {
        for (size_t j = 0; j < N + 1; j++) {
            auto vertex = &vertices[(i * (N + 1) + j) * attrCount];
            vertex[0] = GLfloat(j) / N - 0.5f;
            vertex[1] = GLfloat(i) / N - 0.5f;
            vertex[2] = vertex[0] + 0.5;
            vertex[3] = vertex[1] + 0.5;
        }
    }
    ...
    glBufferData(GL_ARRAY_BUFFER, verticesCount * attrCount * sizeof(GLfloat), vertices, GL_STATIC_DRAW);
...
void createTexture()
{
    ...
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
    // Load, create texture and generate mipmaps
    image = SOIL_load_image("textures/fire.jpg", &width, &height, 0, SOIL_LOAD_RGB);
    glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB, width, height, 0, GL_RGB, GL_UNSIGNED_BYTE, image);
    SOIL_free_image_data(image);
    glBindTexture(GL_TEXTURE_2D, 0);
}
...
void draw(GLfloat p)
{
    ...
    glBindTexture(GL_TEXTURE_2D, texture1);
    glUniform1i(glGetUniformLocation(g_shaderProgram, "u_map1"), 0);

    glActiveTexture(GL_TEXTURE1);
    glBindTexture(GL_TEXTURE_2D, texture2);
    glUniform1i(glGetUniformLocation(g_shaderProgram, "u_map2"), 1);
